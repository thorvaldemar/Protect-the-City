				...
				if (args[0].equalsIgnoreCase("add")) {
					if (Bukkit.getPlayer(args[1]) != null) {
						...
						if (Selector.addPlayer(to)) {
							...
							p.sendMessage(ChatColor.GREEN + "The selector is now active for " + ChatColor.DARK_GREEN + to.getName());
						} else {
							p.sendMessage(ChatColor.DARK_RED + to.getName() + ChatColor.RED + " is already active");
						}
					} else {
						p.sendMessage(ChatColor.DARK_RED + args[1] + ChatColor.RED + " does not exist or is not online");
					}
					...
					if (Bukkit.getPlayer(args[1]) != null) {
						...
						if (Selector.removePlayer(to)) {
							...
							p.sendMessage(ChatColor.RED + "The selector is now deactivated for " + ChatColor.DARK_RED + to.getName());
						} else {
